...
const ActivityFeedItem: React.FC<{ activity: SocialActivity; onClickUser: (uid: string) => void; onDelete?: (id: string) => void }> = ({ activity, onClickUser, onDelete }) => {
    const { theme } = useTheme(); const { user } = useAuth(); const [isMenuOpen, setIsMenuOpen] = useState(false); const [isEditing, setIsEditing] = useState(false); const [editContent, setEditContent] = useState(activity.description || ''); const [isSaving, setIsSaving] = useState(false); const [showReactionPicker, setShowReactionPicker] = useState(false); const menuRef = useRef<HTMLDivElement>(null); const reactionPickerRef = useRef<HTMLDivElement>(null);
    ...
    const handleReaction = async (emoji: string) => { if (!user) return; setShowReactionPicker(false); try { await toggleReaction(activity.id, user.uid, emoji); } catch (e) { console.error(e); } };
...
const RankingsView: React.FC = () => {
    ...
    return ( 
        <div style={{ display: 'flex', flexDirection: 'column', gap: theme.spacing.large }}> 
            ...
            {isLoading && ranking.length === 0 ? <div style={{ display: 'flex', justifyContent: 'center' }}><Loader /></div> : ( 
                <div> 
