...
const TablePage: React.FC = () => {
  ...
  const tutorialSteps = [
    {
        title: 'TABLA anual',
        content: 'Tu carrera temporada a temporada. Analiza puntos, efectividad y promedios.',
        icon: <TableIcon size={48} />,
    },
    {
        title: 'Perfil de RADAR',
        content: 'Visualiza tu evolución. Activa o desactiva años en la leyenda para comparar tu estilo de juego.',
  ...
  const tournamentData = useMemo(() => {
    ...
    matches.forEach(match => {
        // Exclude Career Mode matches from this specific table
        const isCareerMatch = match.matchMode === 'world-cup' || match.matchMode === 'qualifiers';
        // Compatibility fix for old data: Filter out if tournament name starts with "Mundial #" or "Elim." even if matchMode isn't set
        const isLegacyCareerName = match.tournament && (match.tournament.startsWith("Mundial #") || match.tournament.startsWith("Elim. "));

        if (match.tournament && !isCareerMatch && !isLegacyCareerName) {
  ...
  const tournamentMaxValues = useMemo(() => {
    ...
    keysToCompare.forEach(key => {
        maxes[key] = Math.max(...tournamentData.map(d => d[key] as number));
    });
    return maxes;
  }, [tournamentData]);
  ...
  const historicalMaxValuesForRadar = useMemo(() => {
    ...
    return radarMetrics.map(metric => {
      if (['matchesPlayed', 'wins', 'draws', 'losses'].includes(metric.key)) {
        return maxMatchesPlayed;
      }
      if (metric.key === 'winRate' || metric.key === 'efectividad') {
        return 100;
      }
      return Math.max(...yearlyData.map(y => y[metric.key] as number), 1);
    });
  }, [yearlyData]);
    ...
    return (
        <main style={styles.container}>
            ...
            <div style={styles.noDataContainer}>
                ...
                <p style={{ marginTop: theme.spacing.medium }}>Juega y registra partidos para ver tu evolución en la tabla.</p>
            </div>
        </main>
    )
  }
    ...
    <main style={styles.container}>
      ...
      <div style={styles.contentWrapper}>
        ...
        <div style={styles.rightColumn}>
            ...
            <h3 style={{...styles.pageTitle, fontSize: theme.typography.fontSize.large, marginTop: 0 }}>Estadísticas por torneos</h3>
            {tournamentData.length > 0 ? (
                ...
                <div style={styles.noDataContainer}>
                    <p>Asigna torneos a tus partidos para ver la clasificación por competición.</p>
                </div>
            )}
        </div>
      </div>
